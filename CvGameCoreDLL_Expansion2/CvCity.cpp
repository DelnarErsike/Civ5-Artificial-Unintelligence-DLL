...
#if defined(AUI_UNIT_PRODUCTION_AI_LUA_FLAVOR_WEIGHTS) || defined(AUI_BUILDING_PRODUCTION_AI_LUA_FLAVOR_WEIGHTS) || defined(AUI_PROJECT_PRODUCTION_AI_LUA_FLAVOR_WEIGHTS) || defined(AUI_PROCESS_PRODUCTION_AI_LUA_FLAVOR_WEIGHTS)
...
bool CvCity::IsCanPurchase(OrderData* pOrder, YieldTypes eCurrency) const
{
...
int CvCity::GetPurchaseCost(OrderData* pOrder, YieldTypes eCurrency) const
{
...
void CvCity::doProduction(bool bAllowNoProduction)
{
	...
	if(isProduction())
	{
		...
		if(getProduction() >= getProductionNeeded() && !isProductionProcess())
		{
			...
			int iOldProdDiff = getRawProductionDifferenceTimes100(true, false);
#endif
			...
			int iNewProd = getRawProductionDifferenceTimes100(true, false) - iOldProdDiff;
...
//	--------------------------------------------------------------------------------
#ifdef AUI_UNIT_EXTRA_IN_OTHER_PLOT_HELPERS
int CvCity::rangeCombatUnitDefense(_In_ const CvUnit* pDefender, const CvPlot* pInPlot, const int iDefenderExtraFortifyTurns) const
{
	if (pInPlot == NULL)
