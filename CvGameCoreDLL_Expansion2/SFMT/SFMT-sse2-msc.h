...
// Delnar: Had to disable this manually because _MSC_VER is independent of the compiler, ie. this code would be enabled in VS2013 when compiling with VC9.0, thus causing errors
#if 0 //_MSC_VER > 1700
 ...
 */
void SFMersenneTwister::sfmt_gen_rand_all()
{
    int i = 0;
	w128_t * pstate = m_sfmt.state;

	__m128i r1 = pstate[SFMT_N - 2].si;
	__m128i r2 = pstate[SFMT_N - 1].si;
    for (; i < SFMT_N - SFMT_POS1; i++) {
 ...
 */
void SFMersenneTwister::gen_rand_array(w128_t * array, int size)
{
	int i = 0;
	int j = 0;
	w128_t * pstate = m_sfmt.state;
...
{
    int i = 0;
	ALIGN16 w128_t * pstate = m_sfmt.state;
